...
function polygon:init(x1, y1, ...)
    ...
    assert(self.count >= 3, "polygon must have at least 3 points")
...
function polygon:_buildNormals()
    local direction = slickmath.side(self.preTransformedVertices[1], self.preTransformedVertices[2], self.preTransformedVertices[3])
